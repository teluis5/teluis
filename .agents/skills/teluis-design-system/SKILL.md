...
## 8. アイコン使用の5原則 (Heroicons & astro-icon)
ブログ全体の視認性と洗練度を保つため、アイコン使用には以下の厳格なルールを設けます。
1. **ファミリーの統一**: `heroicons` の「Outline（線画）」スタイルのみを使用する（Solidなどは混在させない）。
2. **配置とサイズの固定**: アイコンをテキストにインラインで添える場合は必ず `class="icon-inline"` を付与する（`width: 1.2em; height: 1.2em; vertical-align: text-bottom;` が適用される）。
3. **ノイズの排除**: 「日付」「カテゴリ」「大見出し」など、読者の視線誘導（標識）として本当に必要な箇所にのみ使用し、ただの装飾としては乱用しない。
4. **色づかい**: 原則としてテキストと同じ色（`currentColor`）を継承させる。カラフルな色は使用しない（クリック可能なタグ等のみ青系を許容）。
5. **セマンティック・マッチ（意味の一致）**: 「強み」には上向き矢印や炎 (`fire`, `sparkles`)、「弱み」には注意マーク (`exclamation-triangle`)、「結論」にはチェック (`check-badge`) など、直感的にテキストの内容が連想できるアイコンを厳選して使用する。**内容とイメージが合致するアイコンが見つからない場合は、無理にアイコンを使わない**（無意味なアイコンはノイズになるため）。

## 9. カラーパレット (global.css)
