...
## 7. 記事内の表（比較表）のレイアウト
- **基本スタイル**: 記事内の表は、枠線やヘッダー背景がある見やすいスタイルで表示されます。見出し行はスクロール時に追従（`position: sticky; top: 57px;`）します。
- **列数に応じた動的スクロール**:
  - **比較対象3つまで（全4列以下）**: 画面幅にフィットして表示されます（`width: 100%`）。左端の列は1文字幅（`3.5rem` ＋ `word-break: break-all`）で固定され、残り3列が均等に配置されます。
  - **比較対象4つ以上（全5列以上）**: `BlogPost.astro` に組み込まれたJSにより、自動的にスクロール可能なラッパー `<div class="table-scroll-wrapper">` で囲まれ、「← 左右にスクロールできます →」というガイドが表示されます。
  - **均等な列幅の維持**: JSにより、表の幅が `calc(3.5rem + (100% - 3.5rem) * (対象数 / 3))` に設定されます。これにより、ブラウザのCSSパーサーが安全に解釈でき、**常に最初の4列分（見出し＋対象3つ）が画面幅（100%）にピッタリ収まり**、5列目以降は同じ列幅を維持したまま画面外にはみ出して横スクロールする美しいレイアウトになります。

## 8. カラーパレット (global.css)
