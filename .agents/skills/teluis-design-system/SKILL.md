...
5. **セマンティック・マッチ（意味の一致）**: 「強み」には上向き矢印や炎 (`fire`, `sparkles`)、「弱み」には注意マーク (`exclamation-triangle`)、「結論」にはチェック (`check-badge`) など、直感的にテキストの内容が連想できるアイコンを厳選して使用する。これにより読者の認知負荷を下げる。
