...
- **列数に応じた動的スクロール**:
  - **比較対象3つまで（全4列以下）**: 画面幅にフィットして表示されます。
  - **比較対象4つ以上（全5列以上）**: `BlogPost.astro` に組み込まれたJS（`table.rows[0].cells.length` で正確に列数を判定）により、自動的にスクロール可能なラッパー `<div class="table-scroll-wrapper">` で囲まれます。
  - **均等な列幅の維持**: JSにより、表の幅が `(列数 * 25)%` に設定され、さらに `table-layout: fixed;` が適用されます。これにより、**常に最初の4列分（見出し＋対象3つ）が画面幅（100%）にピッタリ収まり**、5列目以降は同じ列幅を維持したまま画面外にはみ出して横スクロール（スクロールガイド付き）する美しいレイアウトになります。
