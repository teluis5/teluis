...
- **列数に応じた動的スクロール**:
  - **比較対象3つまで（全4列以下）**: 画面幅にフィット（縮小）して表示されます。
  - **比較対象4つ以上（全5列以上）**: `BlogPost.astro` に組み込まれたJSにより、自動的にスクロール可能なラッパー `<div class="table-scroll-wrapper">` で囲まれます。また、表の上に「← 左右にスクロールできます →」というガイド（`table-scroll-hint`）が表示され、`min-width: 800px` が適用されることで強制的に横スクロールが発生します。
